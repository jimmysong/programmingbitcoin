...
Addresses are data that fit into known script templates like that above. Wallets know how to interpret various address types (p2pkh, p2sh, p2wpkh) and create the appropriate ScriptPubKey. All of the above have a particular type of address format (base58, bech32) so people can pay to them.
...
Only when Pieter Wuille discovered that the compressed SEC format existed in OpenSSL did it start being used in Bitcoin.
****
...
The ScriptPubKey and ScriptSig combine to make a list of items for processing that looks like this:
