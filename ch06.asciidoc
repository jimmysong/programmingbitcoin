...
A typical operation is `OP_DUP` (<<op_dup_duplicates_the_top_element>>), which will duplicate the top element (consuming 0) and push the new element to the stack (pushing 1).

[[op_dup_duplicates_the_top_element]]
.`OP_DUP` duplicates the top element
image::images/prbc_0603.png[OP_DUP]

At the end of evaluating all the instructions, the top element of the stack must be nonzero for the Script to resolve as valid.
...
Another very important operation is `OP_CHECKSIG` (<<op_checksig_checks_if_the_signature_for_the_pubkey_is_valid_or_not>>).
...
There are three specific opcodes for handling elements with length greater than 75: `OP_PUSHDATA1`, `OP_PUSHDATA2`, and `OP_PUSHDATA4`.
...
To evaluate the two together, we take the instructions from ScriptSig and ScriptPubKey and combine them as in <<scriptpubkey_and_scriptsig>>.
...
The ScriptPubKey and ScriptSig combine to make an instruction set that looks like <<p2pk_combined>>.

[[p2pk_combined]]
.p2pk Combined
image::images/prbc_0609.png[P2PK Combination]

The two columns in <<p2pk_start>> are instructions of Script and the elements stack.
...
In <<p2pk_start>>, we start with the instructions as combined in <<p2pk_combined>>.
...
`OP_CHECKSIG` consumes two stack instructions (pubkey and signature) and determines if they are valid for this transaction.
...
If this transaction instead had an invalid signature, the result from `OP_CHECKSIG` would be zero, ending our Script processing like <<p2pk_end>>:
...
The ScriptPubKey and ScriptSig combine to a list of instructions that looks like <<p2pkh_combined>>:
...
We start with the instructions as above (<<p2pkh_start>>).

[[p2pkh_start]]
.p2pkh Start
image::images/prbc_0618.png[P2PKH Start]

The first two instructions are elements, so they are pushed to the stack (<<p2pkh_step_1>>).
...
Script evaluation will start like <<example_start>>:
...
`OP_ADD` will consume the top two elements of the stack, add them together and push the sum to the stack (<<example_step_3>>).

[[example_step_3]]
.Example Step 3
image::images/prbc_0631.png[Example 1 Step 3]

`OP_9` will push a 9 to the stack (<<example_step_4>>).

[[example_step_4]]
.Example Step 4
image::images/prbc_0632.png[Example 1 Step 4]

`OP_EQUAL` will consume 2 elements and push a 1 if equal, 0 if not (<<example_end>>).
