...
We start with the instructions as combined above (<<p2pk_start>>):
...
This is again, data is pushed to the stack (<<pspk_step_2>>).
...
Assuming that the signature is valid for this public key, we have this situation (<<pspk_step_3>>):
