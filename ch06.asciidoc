...
Specifying((("ScriptPubKey"))) where the Bitcoins go is the job of the ScriptPubKey.
...
Pay-to-pubkey-hash (p2pkh) is((("p2pkh (pay-to-pubkey-hash)", id="p2pkh06")))((("p2pk (pay-to-pubkey)", "solving problems with p2pkh"))) an alternative Script that has advantages over p2pk:
...
The major advantage is that the ScriptPubKey is shorter (just 25 bytes) and a thief would not only have to solve the Discrete Log problem in ECDSA, but also figure out a way to find pre-images of both ripemd160 and sha256.((("", startref="p2pkh06")))((("", startref="Sp2pkh06")))
...
The((("Script", "utility of"))) previous exercise was a bit of a cheat as `OP_MUL` is no longer allowed on the Bitcoin network.
...
In((("SHA-1 piñata")))((("Script", "SHA-1 piñata"))) 2013, Peter Todd created a Script very similar to the Script in Exercise 4 and put some Bitcoins into it to create an economic incentive for people to find hash collisions.
