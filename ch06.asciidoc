...
In Bitcoin, Script is the smart contract langauge, or the programming language used to express the conditions under which bitcoins are spendable.
...
Specifically, Script ovids any mechanism for loops and is therefore not Turing complete.
...
This would be a large systematic vulnerabilty and protecting against this vulnerability is one of the major reasons why Turing-completeness is avoided.
...
Operations do something to the data.
...
A typical operation might be something like OP_DUP, which will duplicate the top element (consuming 0) and pushing the new element to the stack (pushing 1).
...
`OP_HASH160` does a sha256 followed by a ripemd160 (aka hash160) to the top element of the stack (consuming 1) and pushing a new element to the stack (pushing 1).
...
Essentially, we have a situation like this:
...
The p2pk ScriptPubKey looks like this:
...
For p2pk, the ScriptSig required to unlock the corresponding ScriptPubKey is just the signature:
...
Assuming that the signature is valid for this public key, we have this situation:
...
<4> 99 and 100 are `OP_IF` and `OP_NOTIF` respectively. They require manipulation of the instructions array based on the top element of the stack.
...
The p2pkh ScriptPubKey, or locking Script, looks like this:
...
The p2pkh ScriptSig, or the unlocking Script, looks like this:
...
The ScriptPubKey and ScriptSig combine to a list of instructions that looks like this:

.p2pkh Combined
image::p2pkh3.png[P2PKH Combination]

At this point, the Script is processed one instruction at a time.
We start with the instructions as above.

.p2pkh Start
image::p2pkh4.png[P2PKH Start]

The first two instructions are elements, so they are pushed to the stack.
...
`OP_HASH160` takes the top element and performs the hash160 operation on it (sha256 followed by ripemd160), creating a 20-byte hash:
...
Here is an example ScriptPubKey:

.Example ScriptPubKey
image::ex1.png[Example 1 ScriptPubKey]

Here's a ScriptSig that will unlock the the ScriptPubKey from Figure 6-25.

.Example ScriptSig
image::ex2.png[Example 1 ScriptSig]

The combined Script will be:

.Example Combined
image::ex3.png[Example 1 Combination]

Script eveluation will start this way:
...
`OP_ADD` will consume the top two elements of the stack, add them together and push the sum to the stack.
...
Version 0.3.5 of Bitcoin disabled a lot of different opcodes as anything that had even a little bit of potential to create vulnerabilties on the network were disabled.

This is just as well since most of the functionality in Script is actually not used much.
From a software maintainence standpoint, this is not a great situation as the code has to be maintained despite its lack of usage.
