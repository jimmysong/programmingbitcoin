...
In any case, this is my way of telling you that Finite Fields are not that hard to learn and require no more background than High School algebra.
...
----
<1> We first check that `num` is actually between `0` and `prime-1` inclusive. If not, we get an invalid Field Element and we raise a `ValueError` which is what we should raise when we get an inappropriate value.
