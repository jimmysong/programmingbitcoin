...
<2> The rest of the `\_\_init\_\_` method simply assigns the initialization values.
<3> The `\_\_eq\_\_` method checks if two objects of class `FieldElement` are equal or not. This is only true when the `num` and `prime` properties are equal. This specific method allows us to do things like this:
...
Write the corresponding method `\_\_ne\_\_` which checks if two `FieldElement` objects are _not equal_ to each other.
...
In the class `FieldElement` we can now define `\_\_add\_\_` and `\_\_sub\_\_` methods. The idea of these methods is that we want something like this to work:
...
In Python we can define what addition means for our class with the `\_\_add\_\_` method. So how do we do this? We combine what we learned above with modulo arithmetic and create a new instance of the class `FieldElement` like so:
...
<3> We have to return an instance of the class, which we can conveniently access with `$$self.__class\_\_`. We pass the two initializing arguments, `num` and `self.prime` as defined in the `\_\_init\_\_` method above.

==== Exercise {counter:exercise}

Write the corresponding `\_\_sub\_\_` method which defines the subtraction of two field elements.
...
[NOTE]
====
The answer to this exercise is why _prime_ fields are the most interesting. No matter what *k* you choose, as long as it's greater than 0, multiplying the entire set by *k* will result in the same set as you started with.

Intuitively this results in every element of a Prime Field being equivalent. In a Composite Field, numbers divisible by the field order don't exhibit this trait.
====

==== Coding Multiplication in Python


In the class `FieldElement` we can now define the `\_\_mul\_\_` method. We want this to work:
...
As we did with addition and subtraction above, we can define what multiplication means for our class with the `\_\_mul\_\_` method.

==== Exercise {counter:exercise}

Write the corresponding `\_\_mul\_\_` method which defines the multiplication of two field elements.

==== Coding Exponentiation in Python

We can do the same for exponentiation, which in Python can be defined with the `\_\_pow\_\_` method. The difference here is that the exponent is *not* a field element, so has to be treated a bit differently. We want something like this to work:
...
Instead of another instance of `FieldElement`, we get an integer. We can code it this way.
...
Since we are operating in prime fields, this will always be true. The other fact that we have to wrap around is that division is really multiplication with the inverse. Thus:
...
Note that in Python3, division is separated into `\_\_truediv\_\_` and `\_\_floordiv\_\_`. The first does normal division, the second does integer division.

==== Exercise {counter:exercise}

Write the corresponding `\_\_truediv\_\_` method which defines the division of two field elements.
