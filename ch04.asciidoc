...
SEC stands for((("Standard for Efficient Cryptography", see="SEC format"))) Standards for Efficient Cryptography, and as the word "Efficient" in the name suggests, it's got minimal overhead.
...
Another((("serialization", "DER Signature format")))((("Signature class", "serialization of")))((("DER Signature format")))((("signing and verification", "serializing signatures"))) class that we need to learn to serialize is `Signature`.
...
The((("Distinguished Encoding Rules", see="DER format"))) standard for serializing signatures (and lots of other things, for that matter) is called DER format.
...
4. Encode `r` as a Big-Endian integer, but prepend with `0x00` byte if `r`'s first byte &ge; `0x80`.
Prepend the resulting length to `r`.
Add this to the result.
5. Append the marker byte `0x02`.
6. Encode `s` as a Big-Endian integer, but prepend with `0x00` byte if `s`'s first byte &ge; `0x80`.
...
It's also possible the first byte could be &ge; 0x80, so part 4 can be at most 33 bytes.
...
For reasons we'll get to in <<chapter_script>>, using this particular very simple script turned out to be both wasteful for storing UTXOs and a little less secure than the scripts in more prominent use now.
...
We((("Base64"))) can use something like Base64, which can express 6 bits per character and becomes 87 characters for uncompressed SEC and 44 characters for compressed SEC.
...
What's((("Bech32 standard"))) much better is the new Bech32 standard, which is defined in BIP0173.
Bech32 uses a 32-character alphabet that's just numbers and lowercase letters except `1`, `b`, `i` and `o`.
These are thus far only used for Segwit (<<chapter_segwit>>).((("", startref="base58")))
====
...
The((("serialization", "address format")))((("address format"))) 264 bits from a compressed SEC format are still a bit too long, not to mention a bit less secure (see <<chapter_script>>).
...
The((("ripemd160 operation"))) process of doing a ((("sha256 operation")))sha256 operation followed by a ripemd160 operation is called a hash160 operation((("hash160 operation"))) in Bitcoin.
We can implement this in _helper.py_.
...
The((("serialization", "WIF format")))((("WIF format"))) private key in our case is a 256-bit number.
...
For((("Wallet Import Format", see="WIF format"))) this purpose, there is a format called WIF, which stands for Wallet Import Format.
...
We now turn to Transactions that we can parse and understand.
