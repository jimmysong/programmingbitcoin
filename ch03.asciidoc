...
Visually, invertibility is easy to see (<<each_point_is_invertible_by_taking_the_reflection_over_the_x_axis>>):
...
We know from Point Addition that A+B=B+A (<<the_line_through_the_points_doesnt_change>>):
...
We know from Point Addition that A+(B+C)=(A+B)+C (see Figure pass:[<a data-type="xref" data-xrefstyle="select:labelnumber"  href="#a_b_c_case_one">#a_b_c_case_one</a> and <a data-type="xref" data-xrefstyle="select:labelnumber" href="#a_b_c_case_two">#a_b_c_case_two</a>])
