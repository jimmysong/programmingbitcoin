...
What((("scalar multiplicaiton", "coding for")))((("Elliptic Curve Cryptography", "scalar multiplication coding"))) we're trying to do with Exercise 5 is this:
...
There's((("binary expansion"))) a cool technique called _binary expansion_ that allows us to perform multiplication in `log2(n)` loops, which dramatically reduces the calculation time for large numbers.
...
To((("PrivateKey class"))) program message signing, we now create a `PrivateKey` class, which will house our secret.
