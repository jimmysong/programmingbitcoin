...
We((("Elliptic Curve Cryptography", "elliptic curves over real numbers"))) discussed in <<chapter_elliptic_curves>> what an Elliptic Curve looks like visually because we were plotting the curve over _real_ numbers.
...
So((("Elliptic Curve Cryptography", "elliptic curves over finite fields", id="ECCfinite03"))) what does an Elliptic Curve over a Finite Field look like?
...
OK
----
<1> `helper` is a module with some very useful utility functions, including the ability to run unit tests individually.((("", startref="ECCfinite03")))

=== Point Addition over Finite Fields

We((("Elliptic Curve Cryptography", "point addition over finite fields", id="ECCpoint03"))) can use all the same equations over Finite Fields, including the linear equation:
...
Because we coded FieldElement in such a way as to define `__add__`, `__sub__`, `__mul__`, `__truediv__`, `__pow__`, `__eq__`, and `__ne__`, we can simply initialize `Point` with `FieldElement` objects and point addition will((("", startref="ECCpoint03"))) work:
...
Because((("Elliptic Curve Cryptography", "scalar multiplication for elliptic curves", id="ECCscalar03"))) we can add a point to itself, we can introduce some new notation:
...
We'll see in <<definingCurveBC>> that when we have numbers that are a lot larger, Discrete Log becomes an intractable problem.((("", startref="ECCscalar03")))
...
What((("Elliptic Curve Cryptography", "scalar multiplication coding"))) we're trying to do with Exercise 5 is this:
...
Now((("", startref="ECsecp03")))((("", startref="secp03")))((("", startref="ECCsecp03"))) checking that the order of _G_ is `n` is trivial:
...
At((("Elliptic Curve Cryptography", "Public Key Cryptography")))((("Public Key Cryptography"))) last, we have the tools that we need to do Public Key Cryptography operations.
...
The((("signing and verification", "signature algorithm")))((("signature algorithm"))) inscribing of the target depends on the _signature algorithm_, and in our case, our signature algorithm is called Elliptic Curve Digital Signature Algorithm, or ECDSA for short.
...
In((("signing and verification", "signature hash")))((("signature hash"))) signature/verification parlance, this is called the _signature hash_.
...
Using SHA-1 twice, or double-SHA-1, is the way to defeat or slow down some forms of this attack.
...
So given a public key, which is a point on the secp256k1 curve and a signature hash, `z`, we can verify whether a signature is valid or not.((("", startref="SAVsign03")))
...
We((("signing and verification", "signature creation", id="SAVsig03"))) can now create a signature using some of the primitives that we have.
...
This makes transactions less malleable (more on that in <<chapter_segwit>>).((("", startref="MTelipcurcrypt03")))((("", startref="SAVsig03")))
