...
The one flaw with this argument is that you're trusting k is actually known by the signer.

u=1, v=1, R=G+P

This is the equivalent of the signer can just painting a target around wherever the arrow landed. How do we prove intent?

This, we can do by making one of the arguments, u have r as a component:

u = f(r)

The simplest function of r is just the identity function:
...
At which point, if you know e, then you can do:

uG+vP=rG+((k-r)/e)P=rG+((k-r)/e)eG=rG+(k-r)G=rG+kG-rG=kG=R=(r,y)

And indeed, this could be a signature algorithm, but it leaves you vulnerable to this:
