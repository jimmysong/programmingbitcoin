...
SEC((("OP_CHECKSIG operation"))) public keys and DER signatures are in the stack when a command like `OP_CHECKSIG` is executed, to make getting the public key `P` and signature `(r,s)` pretty straightforward (see <<chapter_script>>).
...
.Quadratic Hashing
====
The((("Quadratic Hashing problem")))((("signing and verification", "signature hash")))((("signature hash")))((("hashes", "signature hashes"))) signature hashing algorithm is inefficient and wasteful and is called the Quadratic Hashing problem.
...
When we view this output on a testnet((("block explorers"))) block explorer (<<utxo_that_were_spending>>), we can see that our output is worth 0.33 tBTC.
