...
SEC public keys and DER signatures are in the stack when an command like `OP_CHECKSIG` is executed, to make getting the public key `P` and signature `(r,s)` pretty straightforward (see <<chapter_script>>).
