...
A light client wouldn't want to reveal, for example, that it has one thousand BTC to a full node.
...
Let's create a Bloom Filter with two items, "hello world" and "goodbye", using the code following (<<ten_bit_bloom_filter_with_two_elements>>).
...
Supposing((("Bloom Filters", "using multiple hash functions"))) that the space of all items is 1 million and we want bucket sizes to still be 5, we would need a Bloom Filter that's 1,000,000 / 5 = 200,000 bits long.
...
The((("murmur3 hash function"))) hash function we use is called `murmur3`.
Unlike sha256, _murmur3_ is not cryptographically secure, but is much faster.
...
A((("Bloom Filters", "getting transactions of interest"))) light client that loads a Bloom Filter with a full node will get all the information needed to prove that transactions of interest are included in particular blocks.

[source,python]
----
include::code-ch12/examples.py[tag=example5]
----
<1> We are creating a Bloom Filter that's 30 bytes, 5 hash functions, using a particularly popular '90s tweak.
...
<7> The `getdata` message asks for two thousand Merkle Blocks after the block defined by `last_block_hex`.
...
What we've done is look at two thousand blocks after a particular block for UTXOs corresponding to a particular address.
