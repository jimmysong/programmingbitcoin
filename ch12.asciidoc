...
Let's create a Bloom Filter with two items, "hello world" and "goodbye" (<<ten_bit_bloom_filter_with_two_elements>>)
...
Conceptually, <<ten_bit_bloom_filter_with_two_elements_and_two_hash_functions>> shows what the code above does:
...
Here is what the payload for `getdata` looks like <<parsed_getdata>>:
