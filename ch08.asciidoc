...
====
It is possible to "split" a single private key into multiple private keys and use an interactive method to aggregate signatures, but this is not a common practice.
...
`OP_0` will push the number `0` to the stack.

.Bare Multisig Step 1
image::multisig5.png[Bare Multisig Step 1]

The signatures are elements so they'll be pushed directly to the stack.

.Bare Multisig Step 2
image::multisig6.png[Bare Multisig Step 2]

`OP_m` will push the number `m` on the stack, the public keys will be pushed to the stack and `OP_n` will push the number `n` to the stack.
...
Assuming that the signatures are valid, the stack has a single `1` which validates the combined Script:
...
In p2sh, a special rule gets executed only when this pattern is encountered:
...
We start with the Script instructions:

.p2sh Start
image::p2sh6.png[p2sh Start]

`OP_0` will push a 0 to the stack, the two signatures and the RedeemScript will be pushed to the stack directly, leading to this:
...
On the other hand, BIP0016 nodes, which as of this writing are the vast majority, will parse the RedeemScript as Script instructions:

.p2sh RedeemScript
image::p2sh2.png[p2sh RedeemScript]

These go into the Script column as instructions:
...
A ScriptPubKey like this would have worked with BIP0012:
