...
We've generalized here so we can see what a Bare `m`-of-`n` Multisig would look like (`m` and `n` can be anything from 1 to 20, though there's numerical opcodes only go((("OP_16 operation"))) up to `OP_16`; 17 to 20 would require `0112` to push a number like 18 to the stack).
...
`OP_m` will((("OP_m operation")))((("OP_n operation"))) push the number `m` on the stack, the public keys will be pushed to the stack, and `OP_n` will push the number `n` to the stack (<<bare_multisig_step_3>>).
...
Pay-to-script-hash (p2sh)((("p2sh (pay-to-script-hash)", "operation of", id="p2shoper08")))((("multisig (multiple signatures)", "p2sh (pay-to-script-hash) operation", id="MSp2shoper08")))((("hashes", "p2sh (pay-to-script-hash)", id="Hp2sh08"))) is a general solution to the long address/ScriptPubKey problem.
...
We've verified one of the two signatures that are required to unlock this p2sh multisig.((("", startref="p2shsignver08")))((("", startref="MSp2shcode08")))((("", startref="Hp2sh08")))
