...
[.lead]
Up to this point in the book, we've been doing single-key transactions, or transactions with only a single private key per input.
...
.`OP_EVAL` would have been an command that adds additional commands based on the top element
