...
All((("networking", "network messages"))) network messages looks like <<network_message_or_the_envelope_that_contains_the_actual_payload>>.
...
The((("networking", "network magic"))) first four bytes are always the same and are referred to as the _network magic_.
...
We((("networking", "headers response messages"))) can now create a node, handshake, and then ask for some headers.
