...
All network messages looks like this:
...
For full nodes, downloading the Block headers allows them to asynchronously ask for full blocks.
...
As with Version, we start with the protocol version, then the number of block header groups in this list, then the starting block header and lastly, the ending block header.
...
The headers command is a list of block headers which we already learned how to parse from <<chapter_blocks>>.
