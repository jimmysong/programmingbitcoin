...
>>> node.handshake()
>>> getheaders = GetHeadersMessage(start_block=GENESIS_BLOCK)
...
...             expected_bits = calculate_new_bits(first_epoch_block, block)
...
We've managed to connect to a node on the network, handshake and do something useful, which is download and verify that the blocks meet the consensus rules. In the next chapter, we focus on getting information about transactions that we're interested in from another node in a private, yet provable way.