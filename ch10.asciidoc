...
[.lead]
The((("peer-to-peer network", seealso="networking"))) peer-to-peer network that Bitcoin runs on is what gives it a lot of its robustness. More than 65,000 nodes are running on the network as of this writing and are communicating constantly.

The Bitcoin network is a((("broadcast networks", seealso="networking")))((("gossip networks", seealso="networking"))) broadcast network or a gossip network.
...
All network messages looks like <<network_message_or_the_envelope_that_contains_the_actual_payload>>.
...
The first four bytes are always the same and are referred to as the _network magic_.
...
There are many different commands; an exhaustive list can be seen at https://en.bitcoin.it/wiki/Protocol_documentation[].
...
2^32^ is about 4 billion, so payloads can be as big as 4GB, though the reference client rejects any payloads over 32MB.
...
As with Version, we start with the protocol version, then the number of block headers in this list (this number can be more than 1 if there's a chain split), then the starting block headers, and the ending block header last.
...
The headers command is a list of block headers (<<parsed_headers_chap_ten>>), which we already learned how to parse from <<chapter_blocks>>.
