...
This should look familiar as this is exactly the state that p2wpkh starts with. After OP_0 and the 20-byte hash we are left with this:
...
Other improvements to Segwit include fixing the quadratic hashing problem through a different calculation of z. Essentially, a lot of the calculations for z can be reused instead of requiring a new double_sha256 hash for each input. The details of the z calculation are in BIP0143.
