...
Pay to witness pubkey hash (p2wpkh) is one of four new types of script that coins can be sent to. This is a smart contract that acts a lot like pay-to-pubkey-hash, but has some of the "unlocking" data elsewhere. The rearrangement is to fix transaction malleability.
...
P2WPKH is great, but unfortunately, this is a new type of script and older wallets who are unaware of the p2wpkh ScriptPubKey format cannot send bitcoins to these ScriptPubKeys. P2wpkh uses bech32 addresses whose ScriptPubKeys older wallets don't know how to create.
