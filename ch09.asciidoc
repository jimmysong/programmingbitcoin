...
It turns out that the ScriptSig of the Coinbase transaction is set by miners. The main restriction is that it has to be at least 2 bytes and no longer than 100 bytes. Other than those restrictions and BIP0034 (see more below) this field can be whatever the miner wants. Here is the ScriptSig for the Genesis Block's Coinbase Transaction:
...
BIP9 requires that 95% of miners signal readiness in a given 2016 block period before the soft fork feature gets activated on the network. The only soft forks to utilize BIP9 as of this writing have been BIP68/BIP112/BIP113 (OP_CHECKSEQUENCEVERIFY and related changes), BIP141 (segwit) and BIP91 (reducing threshold for segwit). They were assigned bits 0, 1 and 4 respectively.
