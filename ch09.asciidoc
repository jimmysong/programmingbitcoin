...
This((("double-spending, preventing"))) is called the _double-spending problem_ and is so called because the owner of a lockbox may try to spend the same output twice.
...
Other than those restrictions and BIP0034 (see <<BIP0034>>) the ScriptSig can be anything the miner wants as long as the evaluation of the ScriptSig by itself, with no corresponding ScriptPubKey, is valid.
Here is the ScriptSig for the Genesis BloBIP0066ck's Coinbase Transaction:
...
=== Headers versus Full Blocks

Blocks((("Blocks", "headers versus full blocks"))) are batches of transactions, and the block header is metadata about the transactions included in the block.
...
Unlike transactions, each field in a block header is of a fixed length, as listed in <<parsed_block>>;
...
The entire blockchain, on the other hand, is roughly 200GB, so the headers are roughly .023% of the size.
...
Version 3 meant that the software was ready for((("BIP (Bitcoin Improvement Proposals)", "BIP0066"))) BIP0066, the enforcement of strict DER encoding.
...
To alleviate this, the developers came up with((("BIP (Bitcoin Improvement Proposals)", "BIP9"))) BIP9, which allows up to 29 different features to be signaled at the same time.

==== BIP9

The way BIP9 works is by fixing the first three bits of the 4-byte (32-bit) header to be `001` to indicate that the miner is using BIP9.
The first three bits have to be `001`, as that forces older clients to interpret the version field as a number greater than or equal to 4, which was the last version number that was used pre-BIP9.

This means that in hexadecimal, the first character will always be 2 or 3.
The other 29 bits can be assigned to different soft-fork features, which miners can signal readiness.
...
BIP9 requires that 95% of blocks signal readiness in a given 2,016 block epoch (the period for a difficulty adjustment; more on that later in this chapter) before the soft fork feature gets activated on the network.
Soft forks that used BIP9 as of this writing have been((("BIP (Bitcoin Improvement Proposals)", "BIP68")))((("BIP (Bitcoin Improvement Proposals)", "BIP112")))((("BIP (Bitcoin Improvement Proposals)", "BIP113"))) BIP68/BIP112/BIP113 (`OP_CHECKSEQUENCEVERIFY` and related changes) and((("BIP (Bitcoin Improvement Proposals)", "BIP141"))) BIP141 (Segwit).
These BIPs used bits 0 and 1 for signaling, respectively.
BIP91((("BIP (Bitcoin Improvement Proposals)", "BIP91"))) used something like BIP9, but used an 80% threshold and a smaller block period, so wasn't strictly using BIP9.
...
----
<1> The `>>` operator((("&gt;&gt; (left bit-shift) operator")))((("left bit-shift (&gt;&gt;) operator"))) is the left bit-shift operator, which throws away the rightmost 29 bits, leaving just the top 3 bits.
...
This timestamp is used in two places: the first for validating timestamp-based Locktimes on transactions included in the block and the second for calculating a new bits/target/difficulty every 2,016 blocks.
The((("BIP (Bitcoin Improvement Proposals)", "BIP113"))) Locktimes were at one point used directly for transactions within a block, but BIP113 changed the behavior to not use the current block's Locktime directly, but((("median-time-past (MTP)"))) the median-time-past (MTP) of the past 11 blocks.
...
Nonce((("Blocks", "nonce field")))((("nonce field"))) stands for "number used only once" or n-once.
This number is what is changed by miners when looking for proof-of-work.

=== Proof-of-Work

Proof-of-work is((("Blocks", "looking for proof-of-work")))((("proof-of-work", "fuction of"))) what secures Bitcoin and, at a deep level, allows the decentralized mining of Bitcoin.
...
Note that each 0 in the hexadecimal above represents four 0-bits.
...
Where((("proof-of-work", "generating new hashes"))) does the miner get new numerical dirt to process to see if it satisfies proof-of-work?
...
Target((("proof-of-work", "difficulty"))) is hard to comprehend for human beings.
...
The mining difficulty in the preceding code is roughly 888 billion times more difficult than when Bitcoin started.
...
The((("time_differential number"))) `time_differential` number is calculated so that if it's greater than 8 weeks, 8 weeks is used, and if it's less than 3.5 days, 3.5 days is used.
...
If each block took on average 10 minutes, 2,016 blocks should take 20,160 minutes.
There are 1,440 minutes per day, which means that 2,016 blocks take 20,160 / 1,440 = 14 days.
...
<3> This part makes sure that if it took less than 3.5 days to find the last 2,015 blocks, we don't increase the difficulty too much.
