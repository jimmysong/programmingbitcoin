...
To prevent a transaction ID collision, the Gavin Andresen authored BIP0034 which is a soft-fork rule that adds the height of the block being mined into the first element of the Coinbase ScriptSig.
...
This rule continues to be needed as optimizations like ASICBoost allow the possibility of the same Coinbase Transaction IDs across multiple blocks.
...
BIP91 was unusual in that it used an 80% threshold and used a smaller block period, so wasn't strictly using BIP9.
