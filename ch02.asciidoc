...
There's a result from the Theorem on the Equality of Polynomials, which states that the coefficients have to equal each other if the roots are the same. The first one that's interesting is the coefficient in front of x^2^:
