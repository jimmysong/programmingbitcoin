...
=== Point Addition for when __P__~1~ = __P__~2~
...
__P__~1~ = –__P__~2~ or __P__~1~ + __P__~2~ = __I__

We've already handled this in Exercise 3.

What happens when __P__~1~ = __P__~2~?
Visually, we have to calculate the line that's *tangent* to the curve at __P__~1~ and find the point at which the line intersects the curve.
...
=== Coding Point Addition for when __P__~1~ = __P__~2~
...
This can only happen if __P__~1~ = __P__~2~ and the y-coordinate is 0, in which case the slope calculation will end up with a 0 in the denominator.
