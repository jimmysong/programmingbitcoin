...
Elliptic Curves((("Elliptic Curves", "definition of", id="ECdefin02"))) are like many equations you've been seeing since pre-algebra.
...
Specifically, the((("Elliptic Curves", "secp256k1 curve")))((("secp256k1 curve"))) Elliptic Curve used in Bitcoin is called secp256k1 and it uses this particular equation:
...
The curve looks like <<secp256k1_curve>>.((("", startref="ECdefin02")))
...
In((("&#x005F;&#x005F;init&#x005F;&#x005F; method"))) other words, `__init__` will raise an exception when the point is not on the curve.((("&#x005F;&#x005F;ne&#x005F;&#x005F; method")))
...
Elliptic Curves((("Elliptic Curves", "Point Addition", id="ECpoint02")))((("Point Addition", "definition of", id="PAdef02"))) are useful because of something called Point Addition.
...
In mathematics parlance, point addition is _nonlinear_.((("", startref="PAdef02")))((("", startref="ECpoint02")))

=== Math of Point Addition

"Addition" in((("Elliptic Curves", "Point Addition math", id="ECpointmath02")))((("Point Addition", "math of", id="PAmath02"))) the name Point Addition satisfies certain properties that we think of as addition, such as:
...
3. Where the two points are the same.((("", startref="ECpointmath02")))((("", startref="PAmath02")))

=== Coding Point Addition

We((("Elliptic Curves", "Point Addition coding")))((("Point Addition", "coding"))) first handle the identity, or the point at infinity.
...
First, we((("&#x005F;&#x005F;init&#x005F;&#x005F; method")))((("&#x005F;&#x005F;add&#x005F;&#x005F; method"))) have to adjust the `__init__` method slightly so it doesn't check that the curve equation is satisfied when we have the point at infinity.
...
Now((("Elliptic Curves", "Point Addition for x&#x2081 &#x22260; x&#x2082;", id="ECxnotequal02")))((("Point Addition", "for x&#x2081 &#x22260; x&#x2082;", id="PAxnotequal02")))((("slope"))) that we've covered the vertical line, let's examine when the points are different.
...
That((("&#x005F;&#x005F;add&#x005F;&#x005F; method"))) means we have to adjust the `__add__` method to handle the case where x~1~≠x~2~.
...
At((("self.__class__ method"))) the end of the method, we return an instance of the class `Point` using `self.__class__` to make subclassing easier.((("", startref="ECxnotequal02")))((("", startref="PAxnotequal02")))

include::code-ch02/answers.py[tag=exercise5,indent=0]

=== Point Addition for When P~1~ = P~2~

When((("Elliptic Curves", "Point Addition for P&#x2018; = P&#x2082;", id="Pequal02")))((("Point Addition", "for P&#x2018; = P&#x2082;", id="PAPequal02"))) the `x` coordinates are the same and the `y` coordinate is different, we have the situation where the points are opposite each other over the x-axis.
...
We((("&#x005F;&#x005F;add&#x005F;&#x005F; method"))) adjust the `__add__` method to account for this particular case.
We have the formulas, we now implement them.((("", startref="Pequal02")))((("", startref="PAPequal02")))
...
There((("Elliptic Curves", "where tangent line is vertical")))((("Point Addition", "where tangent line is vertical"))) is one more exception and this involves the case where the tangent line is vertical (<<vertical_and_tangent_to_the_curve>>).
...
We now combine the concepts from <<chapter_finite_fields>> and <<chapter_elliptic_curves>> to learn Elliptic Curve Cryptography in <<chapter_elliptic_curve_cryptography>>.((("", startref="MTeliptic02")))
