...
1. If the node's value is given in the hashes field (blue box in the <<processing_a_merkle_block>>), the flag bit is 0.
2. If the node's value is an internal node and the value is to be calculated by the light client (dotted outline in the <<processing_a_merkle_block>>), the flag bit is 1.
3. If the node is a leaf node and is a transaction of interest (green box in the <<processing_a_merkle_block>>), the flag is 1 and also given in the hashes field.
