...
The prerequisites are an ordered list of items and a hash function with pre-image resistance.
...
Visually, a Merkle Tree looks like this:
...
Light nodes can get proofs that transactions of interest were included in a block without having to know all the transactions of a block.
...
In other words, light client security is based on a robust network of nodes andthe economic cost of producing proof-of-work.
...
Breadth-first traversal would go level by level like this:
...
In a Proof of Inclusion, the full node sends the green boxes, H~K~ and H~N~ along with the blue boxes H~ABCDEFGH~, H~IJ~, H~L~, H~M~ and H~OP~.
...
The `merkleblock` network command is what communicates this information and looks like this:
...
We don't need to traverse any of the descendents of H~ABCDEFGH~ and go straight to H~IJKLMNOP~ instead.
...
Finall, we traverse to H~ABCDEFGHIJKLMNOP~ which is the Merkle Root and calculate it!
...
----
<1> The point of populating this Merkle Tree is to calcuate the root.
...
The main drawback of SPV is that the nodes you are connecting to know something about the transactions you are intersted in.
