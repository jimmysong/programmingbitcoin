...
Each input contains 4 fields, the first two which point to the previous transaction output and two more that define how it can be spent. These are as follows:
...
----
    ...
    def value(self, testnet=False):
        '''Get the output value by looking up the tx hash on libbitcoin server
