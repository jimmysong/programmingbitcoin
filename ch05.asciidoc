...
With this in mind, we can start constructing the((("Transaction class (Tx)", "creating"))) Transaction class, which we'll call `Tx`:
...
When((("Transactions", "version component")))((("version numbers"))) you see a version number in something (<<version_chap_five>> shows an example), it's meant to give the receiver information about what the versioned thing is supposed to represent.
...
We'll((("Script objects", "creating"))) delve more deeply into how Script is parsed in <<chapter_script>>, but for now, here's how you get a `Script` object from hexadecimal in Python:
...
.UTXO Set
****
UTXO((("UTXO Set")))((("Unspent Transaction Output", see="UTXO Set"))) stands for Unspent Transaction Output.
...
We((("TxFetcher class", "creating"))) are creating a new class to handle this, called `TxFetcher`:
...
[WARNING]
====
As((("third parties, minimizing risk of"))) Nick Szabo eloquently wrote in his seminal essay https://nakamotoinstitute.org/trusted-third-parties/["Trusted Third Parties are Security Holes"], trusting third parties to provide correct data is _not_ a good security practice.
