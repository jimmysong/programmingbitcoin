...
In Bitcoin's case, the transaction version is generally 1, but there are cases where it can be 2 (transactions((("OP_CHECKSEQUENCEVERIFY operation"))) using an opcode called `OP_CHECKSEQUENCEVERIFY` as defined in((("BIP (Bitcoin Improvement Proposals)", "BIP0112"))) BIP112 require use of version > 1).
